...
Ferns::Ferns(int n, int maxDepth, const float photoThresh, const int resolution_w,
        const int resolution_h, const CameraModel& intr)
    : num(n),
      factor(8),
      maxDepth(maxDepth),
      photoThresh(photoThresh),
      width(resolution_w / factor),
      height(resolution_h / factor),
      intr_cx(intr.cx),
      intr_cy(intr.cy),
      intr_fx(intr.fx),
      intr_fy(intr.fy),
      ...
      lastClosest(-1),
      badCode(255),
      rgbd(resolution_w / factor, resolution_h / factor, intr_cx / factor,
           intr_cy / factor, intr_fx / factor, intr_fy / factor),
...
float Ferns::photometricCheck(const float* vertSmall, const Img<Eigen::Matrix<unsigned char, 3, 1>>& imgSmall,
  ...
  float invfx = 1.0f / float(intr_fx / factor);
