...
bool Ferns::addFrame(GPUTexture * imageTexture, GPUTexture * vertexTexture, GPUTexture * normalTexture, const Eigen::Matrix4f & pose, int srcTime, const float threshold)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        if(verts.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) > 0)
        {
          ...
          // std::cout << "pixel0 : " << pix(0) << ", conservatory : "  << conservatory.at(i).rgbd(0) << std::endl;
          // std::cout << " conservatory.at(i).rgbd(0) : " << conservatory.at(i).rgbd(0) << std::endl;
          // std::cout << " (pix(0) > conservatory.at(i).rgbd(0)) : " << (pix(0) > conservatory.at(i).rgbd(0)) << std::endl;
            ...
            // std::cout << " conservatory.at(i).ids[code].size() : " << conservatory.at(i).ids[code].size() << std::endl;
            // std::cout << " coOccurrences : " << coOccurrences[0] << std::endl;
            // std::cout << " code : " << code << std::endl;
            for(size_t j = 0; j < conservatory.at(i).ids[code].size(); j++)
            {
                coOccurrences[conservatory.at(i).ids[code].at(j)]++;
            // std::cout << "conservatory.at(i).ids[code] : " << (conservatory.at(i).ids[code]) << std::endl;
            // std::cout << " conservatory.at(i).ids[code].size() : " << (conservatory.at(i).ids[code].size()) << std::endl;
            // std::cout << " conservatory.at(i).ids[code].at(j) : "  << (conservatory.at(i).ids[code].at(j)) << std::endl;
            // std::cout << " coOccurrences : " << coOccurrences[0] << std::endl;
            // int khk;
            // std::cin>>khk; 
            }
    ...
    if(frame->goodCodes > 0)
    {
      ...
      // std::cout << " maxCo : " << maxCo << " , frames.at(i)->goodCodes : " << frames.at(i)->goodCodes<< " , frame->goodCodes : "<< frame->goodCodes << std::endl;

      //       std::cout << " minimum : " << minimum << " , dissim : " << dissim << " , coOccurrences[i] : " << coOccurrences[i] << std::endl; 
  ...
  // std::cout << " good code : " << frame->goodCodes  << ", frames_size : "<< frames.size() << " ,minimum : " <<  minimum << std::endl;

    if((minimum > threshold || frames.size() == 0) && frame->goodCodes > 0)
    {
        // std::cout << " frame added " << frame->goodCodes << std::endl;
        for(int i = 0; i < num; i++)
        {
    ...
    {
        delete frame;
        // std::cout << " not adding frame " << std::endl;
        return false;
    }
}
                                 ...
                                 const bool lost)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        {
            // std::cout << " shifed  " << conservatory.at(i).rgbd(0) << 3 << "    ---- " << conservatory.at(i).rgbd(0) << std::endl;
            ...
            // std::cout << pix(0) << " ---- " << (pix(0) > conservatory.at(i).rgbd(0)) << std::endl;
            // int abc;
            // std::cin >> abc;
            code = (pix(0) > conservatory.at(i).rgbd(0)) << 3 |
                   (pix(1) > conservatory.at(i).rgbd(1)) << 2 |
    ...
    {
        // std::cout << " HD : " << blockHDAware(frame, frames.at(minId)) << std::endl;
...
//        rgbd.initRGB(&colorCurrent);
       ...
       // std::cout <<  "  fern : odom   "  <<rgbd.lastICPError << ", " << rgbd.lastICPCount << ", " << photoError << std::endl;
        ...
        {
          std::cout << " looop closure detected " << std::endl;
          //                 int abc;
          // std::cin >> abc;
            lastClosest = minId;
