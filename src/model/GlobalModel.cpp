...
GlobalModel::GlobalModel(int width, int height, CameraModel intr)
 : target(0),
   ...
   count(0),
   initProgram(loadProgramFromFile("init_unstable.vert", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   // drawProgram(loadProgramFromFile("draw_feedback.vert", "draw_feedback.frag", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   // drawSurfelProgram(loadProgramFromFile("draw_global_surface.vert", "draw_global_surface.frag", "draw_global_surface.geom", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   dataProgram(loadProgramFromFile("data.vert", "data.frag", "data.geom", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   updateProgram(loadProgramFromFile("update.vert", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   unstableProgram(loadProgramGeomFromFile("copy_unstable.vert", "copy_unstable.geom", "/home/developer/work/git_push/new/slam/src/model/shaders/")),
   ...
   intr(intr)
{
    ...
    vertices = new float[numPixels * Vertex::SIZE];

    memset(&vertices[0], 0, numPixels * Vertex::SIZE);

    glGenTransformFeedbacks(1, &newUnstableFid);
    glGenBuffers(1, &newUnstableVbo);
    glBindBuffer(GL_ARRAY_BUFFER, newUnstableVbo);
    glBufferData(GL_ARRAY_BUFFER, numPixels * Vertex::SIZE, &vertices[0], GL_STREAM_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
                       ...
                       const float weighting)
{
    ...
    dataProgram->setUniform(Uniform("cam", Eigen::Vector4f(intr.cx,
                                                     intr.cy,
                                                     1.0 / intr.fx,
                                                     1.0 / intr.fy)));
    dataProgram->setUniform(Uniform("cols", (float)width));
                        ...
                        const bool isFern)
{
    ...
    unstableProgram->setUniform(Uniform("cam", Eigen::Vector4f(intr.cx,
                                                         intr.cy,
                                                         intr.fx,
                                                         intr.fy)));
    unstableProgram->setUniform(Uniform("cols",(float) width));
    unstableProgram->setUniform(Uniform("rows",(float) height));
