...
void RGBDOdometry::initICP(cudaArray* predictedVertices, cudaArray* predictedNormals, const float depthCutoff) {

  cudaMemcpyFromArray(vmaps_tmp.ptr(), predictedVertices, 0, 0, vmaps_tmp.sizeBytes(), cudaMemcpyDeviceToDevice);

  cudaMemcpyFromArray(nmaps_tmp.ptr(), predictedNormals, 0, 0, nmaps_tmp.sizeBytes(), cudaMemcpyDeviceToDevice);
...
void RGBDOdometry::initICPModel(cudaArray* predictedVertices, cudaArray* predictedNormals, const float depthCutoff,
...
void RGBDOdometry::populateRGBDData(cudaArray* rgb, DeviceArray2D<float>* destDepths, DeviceArray2D<unsigned char>* destImages) {
  verticesToDepth(vmaps_tmp, destDepths[0], maxDepthRGB);

  for (int i = 0; i + 1 < NUM_PYRS; i++) pyrDownGaussF(destDepths[i], destDepths[i + 1]);

  imageBGRToIntensity(rgb, destImages[0]);
...
void RGBDOdometry::initRGBModel(cudaArray* rgb) {
...
void RGBDOdometry::initRGB(cudaArray* rgb) {
...
void RGBDOdometry::initFirstRGB(cudaArray* rgb) {
  imageBGRToIntensity(rgb, lastNextImage[0]);
  // rgb->cudaUnmap();
