...
void DeformationGraph::setPosesSeq(std::vector<unsigned long long int>* poseTimeMap, const std::vector<Eigen::Matrix4f>& poses) {
  ...
  for (unsigned int i = 0; i < poses.size(); i++) {
    ...
    if (abs(int(sampledGraphTimes[imin] - poseTime)) <= abs(int(sampledGraphTimes[imid] - poseTime)) &&
        abs(int(sampledGraphTimes[imin] - poseTime)) <= abs(int(sampledGraphTimes[imax] - poseTime))) {
      foundIndex = imin;
    } else if (abs(int(sampledGraphTimes[imid] - poseTime)) <= abs(int(sampledGraphTimes[imin] - poseTime)) &&
               abs(int(sampledGraphTimes[imid] - poseTime)) <= abs(int(sampledGraphTimes[imax] - poseTime))) {
      foundIndex = imid;
    } else {
      foundIndex = imax;
    }
...
void DeformationGraph::weightVerticesSeq(std::vector<unsigned long long int>* vertexTimeMap) {
  ...
  for (unsigned int i = lastPointCount; i < sourceVertices->size(); i++) {
    ...
    if (abs(int(sampledGraphTimes[imin] - vertexTime)) <= abs(int(sampledGraphTimes[imid] - vertexTime))&&
        abs(int(sampledGraphTimes[imin] - vertexTime)) <= abs(int(sampledGraphTimes[imax] - vertexTime))) {
      foundIndex = imin;
    } else if (abs(int(sampledGraphTimes[imid] - vertexTime)) <= abs(int(sampledGraphTimes[imin] - vertexTime)) &&
               abs(int(sampledGraphTimes[imid] - vertexTime)) <= abs(int(sampledGraphTimes[imax] - vertexTime))) {
      foundIndex = imid;
    } else {
      foundIndex = imax;
    }
