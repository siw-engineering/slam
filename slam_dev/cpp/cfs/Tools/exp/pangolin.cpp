

#include <pangolin/gl/glsl.h>
#include <pangolin/pangolin.h>
#include <pangolin/window_frameworks.h>
#include <pangolin/display/display.h>
#include <pangolin/display/display_internal.h>
#include <iostream>
#include "Uniform.h"



class Shader : public pangolin::GlSlProgram {
 public:
  Shader() {}

  GLuint programId() { return prog; }

  void setUniform(const Uniform& v) {
    GLuint loc = glGetUniformLocation(prog, v.id.c_str());

    switch (v.t) {
      case Uniform::INT:
        glUniform1i(loc, v.i);
        break;
      case Uniform::UINT:
        glUniform1ui(loc, v.ui);
        break;
      case Uniform::FLOAT:
        glUniform1f(loc, v.f);
        break;
      case Uniform::VEC2:
        glUniform2f(loc, v.v2(0), v.v2(1));
        break;
      case Uniform::VEC3:
        glUniform3f(loc, v.v3(0), v.v3(1), v.v3(2));
        break;
      case Uniform::VEC4:
        glUniform4f(loc, v.v4(0), v.v4(1), v.v4(2), v.v4(3));
        break;
      case Uniform::MAT4:
        glUniformMatrix4fv(loc, 1, false, v.m4.data());
        break;
      default:
        assert(false && "Uniform type not implemented!");
        break;
    }
  }
};

static inline std::shared_ptr<Shader> loadProgramFromFile(const std::string& vertex_shader_file, const std::string& fragment_shader_file,
                                                          const std::string& geometry_shader_file) {
  std::shared_ptr<Shader> program = std::make_shared<Shader>();
  std::string shader_dir;
  shader_dir = "/home/christie/projects/work/siw/slam/slam_dev/cfs/Core/Shaders";
  std::cout<<"its allright here";
  program->AddShaderFromFile(pangolin::GlSlVertexShader, shader_dir + "/" + vertex_shader_file, {},
                             {shader_dir});
  program->AddShaderFromFile(pangolin::GlSlFragmentShader, shader_dir + "/" + fragment_shader_file, {},
                             {shader_dir});
  program->Link();

  return program;
}


int main()
{
	std::shared_ptr<Shader> program = std::make_shared<Shader>();

	pangolin::CreateWindowAndBind("Main", 640, 480, pangolin::Params({{"scheme", "headless"}}));


	program = loadProgramFromFile("empty.vert", "resize.frag", "quad.geom");


}

